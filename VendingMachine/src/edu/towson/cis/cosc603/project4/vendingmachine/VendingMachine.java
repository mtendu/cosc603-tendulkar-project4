...
public class VendingMachine {
	...
	// "is empty" part of error message for purchase
	private static final String IS_EMPTY = "is empty -- cannot make purchase";
	
	// Insufficient Balance
	private static final String INSUFFICIENT_BALANCE = "cannot make purchase -- insufficient balance";
	...
	/**
	 * Gets the item occupying t.he slot with the given code.
	...
	public boolean makePurchase(String code) throws VendingMachineException{
		...
		if(item == null){
			throw new VendingMachineException(SLOT_MESSAGE + code + IS_EMPTY );
		}	
		if(this.balance <= item.getPrice()){
			throw new VendingMachineException(INSUFFICIENT_BALANCE);
		}
		else {
			removeItem(code);
